...
private Q_SLOTS:
    void testReservation()
    {
        ...
        QVERIFY(ev);
        QCOMPARE(ev->matrixType(), "org.kde.itinerary.reservation"_L1);
        QCOMPARE(ev->stateKey(), batchId);
        QVERIFY(!ev->contentJson().isEmpty());

        QCOMPARE(MatrixSyncContent::readBatch(ev.get(), &mgr), batchId);
        QCOMPARE(mgr.batches().size(), 2);
        ...
        QCOMPARE(MatrixSyncContent::readBatch(ev.get(), &mgr), QString());
        QCOMPARE(mgr.batches().size(), 1);
        QCOMPARE(mgr.hasBatch(batchId), false);
    }
    ...
    void testLiveData()
    {
        ...
        QVERIFY(evEmpty);
        QCOMPARE(evEmpty->matrixType(), "org.kde.itinerary.livedata"_L1);
        ...
        QVERIFY(evData);
        QCOMPARE(evData->matrixType(), "org.kde.itinerary.livedata"_L1);
        QCOMPARE(evData->stateKey(), trainLeg1);
        QVERIFY(!evData->contentJson().isEmpty());

        MatrixSyncContent::readLiveData(evData.get(), &ldm);
        QCOMPARE(ldm.arrival(trainLeg1).arrivalDelay(), 2);

        MatrixSyncContent::readLiveData(evEmpty.get(), &ldm);
