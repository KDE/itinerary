...
QJsonObject MatrixSyncLocalStateChange::toJson(const MatrixSyncLocalStateChange &change)
{
    return QJsonObject{{
...
std::optional<MatrixSyncLocalStateChange> MatrixSyncLocalStateChange::fromJson(const QJsonObject &obj)
{
    bool ok = false;
    ...
    return MatrixSyncLocalStateChange{
...
bool MatrixSyncLocalStateChange::operator==(const MatrixSyncLocalStateChange &other) const
{
...
void MatrixSyncLocalStateQueue::append(ChangeType type, const QString &id, const QString &context)
{
    ...
    MatrixSyncLocalStateChange change{ .type = type, .id = id, .context = context};
...
void MatrixSyncLocalStateQueue::load()
{
    ...
    for (const auto &obj : array) {
        auto c = MatrixSyncLocalStateChange::fromJson(obj.toObject());
        if (c) {
...
void MatrixSyncLocalStateQueue::store()
{
    if (m_pendingChanges.empty()) {
        ...
        for (const auto &change :m_pendingChanges) {
            array.push_back(MatrixSyncLocalStateChange::toJson(change));
        }
        QFile f(path());
