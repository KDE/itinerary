...
{
    // characters that would need some form of quoting in JSON (and thus increase the size)
    const auto specialChars = std::ranges::count_if(content, [](auto c) { return c < 20 || c == '"' || c >= 127; });

    const auto jsonSize = content.size() - specialChars + (specialChars * 4);
...
std::optional<MatrixSyncStateEvent> MatrixSyncStateEvent::fromQuotient(const Quotient::StateEvent &state)
{
    bool found = false;
