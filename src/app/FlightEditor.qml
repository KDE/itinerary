...
EditorPage {
    ...
    function apply(reservation) {
        ...
        const flightNum = flightNumber.text.match(/^\s*([A-Z0-9]{2}[A-Z]?)\s*(\d{1,5})\s*$/);
        if (flightNum) {
    ...
    ColumnLayout {
        ...
        FormCard.FormCard {
            ...
            FormCard.FormTextFieldDelegate {
                ...
                text: root.reservation.reservationFor.airline.iataCode + " " + root.reservation.reservationFor.flightNumber
                status: Kirigami.MessageType.Warning
                statusMessage: flightNumber.text.match(/^\s*([A-Z0-9]{2}[A-Z]?)\s*(\d{1,5})\s*$/) ? "" : i18n("Invalid flight number.")
            }
        }
